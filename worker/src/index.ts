...
async function main() {
    ...
    await consumer.run({
        ...
        eachMessage: async ({ topic, partition, message }) => {
            ...
            const zapRunMetadata = zapRunDetails?.metadata as any;
            ...
            try {
                const parseContext: Record<string, any> = { ...(zapRunMetadata as any), body: zapRunMetadata };
                if (zapRunMetadata && typeof zapRunMetadata === "object" && "steps" in zapRunMetadata) {
                    const stepsObj = (zapRunMetadata as any).steps;
                ...
                if (currentAction.type.id === "send-email") {
                    ...
                    const body = parse((metadata)?.body as string, parseContext);
                    const to = parse((metadata)?.email as string, parseContext);
                    ...
                    const url = parse((metadata)?.url as string, parseContext);

                    const headers: Record<string, string> = {};
                    const rawHeaders = (metadata)?.headers as Record<string, any>;
                    if (rawHeaders) {
                    ...
                    const auth = (metadata)?.auth as any;
                    if (auth) {
                    ...
                    const rawPrompt = (metadata)?.prompt as string;
                    const schema = (metadata)?.schema as Record<string, string>;
                    ...
                    const rawUrl = (metadata)?.webhookUrl as string;
                    const rawContent = (metadata)?.content as string;
                ...
                const currentMetadata = currentRunDetails?.metadata as Record<string, any> || {};

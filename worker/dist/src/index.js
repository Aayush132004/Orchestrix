    ...
    await consumer.run({
        ...
        eachMessage: async ({ topic, partition, message }) => {
            ...
            if (currentAction.type.id === "send-email") {
                ...
                const body = parse(currentAction.metadata?.body, zapRunMetadata); //this value be somthing like ==> you just receives {comment.amount}
                const to = parse(currentAction.metadata?.email, zapRunMetadata); //{comment.email}
            ...
            if (currentAction.type.id === "send-sol") {
                ...
                console.log(`Sending out SOL to ${amount} to address ${address}`);
            }
            //wait for 5ms

            ...
            <div className="w-full min-h-screen bg-slate-200 flex flex-col justify-center">
                ...
                <div className="w-full pt-2 pb-2">
                    {selectedActions.map((action, index) => (
                        <div key={action.index || index} className="pt-2 flex justify-center"> 
                            <ZapCell 
                ...
                <div className="relative bg-white rounded-lg shadow">
                    ...
                    <div className="p-4 md:p-5 space-y-4">
                        ...
                        {step===0&&availableItems.map(({ id, name, image }) => (
                            ...
                            //if its a trigger select it directly 
                            //else if action than extract info from metadata then select
                            if(isTrigger){
                                onSelect({
                                    id,
                                    name,
                                 ...
                                 //on which step now currently in flow
                                }
                            
                            }} className="flex border p-4 cursor-pointer hover:bg-slate-100 items-center gap-4">
...
function EmailSelector({setMetadata}:{
    ...
    return <div>
       <Input  onChange={(e)=>setEmail(e.target.value)} label={"To"} type={"text"} placeholder="To"></Input>
       <Input  onChange={(e)=>setBody(e.target.value)} label={"Body"} type={"text"} placeholder="Body"></Input>
        <PrimaryButton onClick={()=>{
            setMetadata({
            email,
            body
...
function SolanaSelector({setMetadata}:{
    ...
    return <div>
    <Input  onChange={(e)=>setAmount(e.target.value)} label={"amount"} type={"text"} placeholder="amount"></Input>
    <Input  onChange={(e)=>setAddress(e.target.value)} label={"address"} type={"text"} placeholder="address"></Input>
     <PrimaryButton onClick={()=>{
        setMetadata({
            amount,
            address
